...
export const VariablePanel: React.FC<Props> = ({ data, options, width, height, eventBus }) => {
  const { tableData, getRowId, columns, getSubRows } = useTable({ data, options, eventBus });
  ...
  return (
    ...
    >
      ...
      {tableData.length > 0 && <Table columns={columns} data={tableData} getRowId={getRowId} getSubRows={getSubRows} />}
    </div>
  );
};
